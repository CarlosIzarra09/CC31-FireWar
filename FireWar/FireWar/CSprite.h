...
public:
	...
	virtual void dibujar(Graphics^ _buffer, Bitmap^ _imagen) {
		_buffer->DrawImage(_imagen, this->areaDibujo);
	}
...
public:
	CSprite(Rectangle _areaDibujo, short _nroSI, short _nroSC, short _indiceX, short _indiceY) :
		CImagen(_areaDibujo) {
		this->nroSI = _nroSI;
		this->nroSC = _nroSC;
	...
	virtual void dibujar(Graphics^ _buffer, Bitmap^ _imagen) override {
		Rectangle recorte = obtenerRecorte(_imagen);
		_buffer->DrawImage(_imagen, this->areaDibujo, recorte, GraphicsUnit::Pixel);
